	...
	public static CancelTaxResponse CancelTax(String baseURL, String username, String password, CancelTaxRequest req){
		...
		try {
			...
			{									//If we got a more serious error, print out the error message.
				CancelTaxResponse res = mapper.readValue(conn.getErrorStream(), CancelTaxResponse.class);  //Deserialize response
			...
			CancelTaxResponse res = mapper.readValue(conn.getInputStream(), CancelTaxResponse.class); //Deserialize response
				if(res.CancelTaxResult.ResultCode.equals(SeverityLevel.Error)) //If we have an error in the response, display the error messages.
				{
				...
				{
					System.out.println(res.CancelTaxResult.ResultCode);   //If everything completed successfully, let us know.
				}
			return res;
			}
